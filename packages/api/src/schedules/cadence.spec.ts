...
describe('cadenceIntervalMinutes', () => {
  it('returns the minimum minutes between occurrences per frequency', () => {
    ...
    // Day-and-longer gaps report the DST-compressed minimum: spring-forward puts
    // consecutive daily occurrences 23h apart, and a floor set at the nominal 1440
    // would admit a schedule that genuinely violates it once a year.
    expect(cadenceIntervalMinutes(cadence({ frequency: 'daily' }))).toBe(24 * 60 - 60);
    expect(cadenceIntervalMinutes(cadence({ frequency: 'weekdays' }))).toBe(24 * 60 - 60);
  });

  it('uses the shortest gap between weekly days, incl. the week wrap-around', () => {
    // single day → full week
    expect(cadenceIntervalMinutes(cadence({ frequency: 'weekly' }))).toBe(7 * 24 * 60 - 60);
