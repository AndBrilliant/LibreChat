...
class GenerationJobManagerClass {
  ...
  async completeJob(
    ...
    if (error) {
      const finalized = await this.jobStore.transitionStatus(streamId, {
        ...
        patch: { ...(options?.preserveForReconcile ? {} : { completedAt: Date.now() }), error },
      });
      if (!finalized) {
  ...
  async abortJob(
    ...
    options?: {
      ...
      /**
       * Generation fence for a caller that decided to abort from an EARLIER observation.
       * The scheduler checks a job's scheduled identity and then awaits, so without this
       * an interactive turn replacing the job at that conversationId in the interval
       * would receive the abort signal, terminal event and cleanup meant for the run.
       * Everything dev fences here derives from the fresh read below, which cannot
       * express "the generation I already looked at".
       */
      expectedCreatedAt?: number;
    },
  ): Promise<AbortResult> {
